...
const customNodeBuilder = ({ node: { data: { kind, path, ...data }, ...node }, children, perf, }) => ((lexicalMap) => mapLexical({
    node,
    children,
    data,
    kind,
...
const mapLexical = ({ node, children, data, kind, path, defaults, lexicalMap }) => {
    ...
    return (((map) => (typeof map === "function" ? map(_defaults) : map))(maps.find((map) => map !== undefined)) ?? _defaults);
};
