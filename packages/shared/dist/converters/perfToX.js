...
export const convertSequence = ({ sequence, sequenceId, nodeBuilder: buildNode }) => {
    ...
    return buildNode({
        ...
        children: blocks?.reduce((convertedBlocks, block, index) => ((convertedBlock) => convertedBlock ? pushToArray(convertedBlocks, convertedBlock) : convertedBlocks)(convertBlock({
            block,
...
export const getContents = ({ content, nodeBuilder: buildNode, path }) => content?.reduce((convertedContentNodes, contentItem, index) => {
    const contentPath = path + `.content[${index}]`;
    return ((convertedContentNode) => convertedContentNode
        ? pushToArray(convertedContentNodes, convertedContentNode)
        : convertedContentNodes)(typeof contentItem === "string"
        ? buildNode({
            path: contentPath,
